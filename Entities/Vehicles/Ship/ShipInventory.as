								...
								if (map.getBlobsInRadius(middle, map.tilesize, @blobsInRadius))
								{
									for (uint i = 0; i < blobsInRadius.length; i++)
									{
										...
										if (!b.isAttached())
										{
											...
											GUI::DrawRectangle(getDriver().getScreenPosFromWorldPos(bpos + Vec2f(width / -2.0f, height / -2.0f)),
											                   getDriver().getScreenPosFromWorldPos(bpos + Vec2f(width / 2.0f, height / 2.0f)),
											                   SColor(0x65ed1202));
										}
									}
								}
